...
- No path → locate the most recent `/blueprint` tasks file (`<spec-stem>/tasks.md`, or a single `<spec>.md`) or `goal.md` charter; **or** use an approved native plan-mode plan / an inline plan / "the diff + what it was meant to do" straight from the conversation. Ask via `AskUserQuestion` only if the intent is genuinely unclear.
...
1. **Resolve inputs** (thin part — keep it cheap). `/verify-done` is **plan-source-agnostic** — use whichever you have:
   - **Structured (best, proof-driven):** a `/blueprint` plan — `<spec-stem>/tasks.md` (+ sibling `<spec-stem>/reference.md`), or a single-file `<spec>.md` — parse `Done when:` lines into `doneWhenProofs = [{id, title, cmd}]`; intent = the `reference.md` (or the single `<spec>.md` itself if not split). Or a `goal.md` charter (proofs from its audit table, intent from its stated outcome). **If the tasks file has a `## Needs your attention` block with unresolved blocking `❓ NEEDS YOU` items, pass them in `intentNotes` — an open blocking decision means the plan isn't acceptable yet; surface each in the verdict's NOT-covered bucket rather than blessing around it.**

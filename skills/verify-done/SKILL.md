...
- No path → locate the most recent `/blueprint` plan (`<spec-stem>/tasks.md` checklist + `<spec-stem>/reference.md` for the `## Task details` proofs, or a single `<spec>.md`) or `goal.md` charter; **or** use an approved native plan-mode plan / an inline plan / "the diff + what it was meant to do" straight from the conversation. Ask via `AskUserQuestion` only if the intent is genuinely unclear.
...
1. **Resolve inputs** (thin part — keep it cheap). `/verify-done` is **plan-source-agnostic** — use whichever you have:
   - **Structured (best, proof-driven):** a `/blueprint` plan. The `## Tasks` checklist (in `<spec-stem>/tasks.md`, or a single `<spec>.md`) lists the tasks; the **`Done when:` proofs + `### TASK-n` blocks live in `<spec-stem>/reference.md` `## Task details`** (or the single file's `## Task details`) — parse those into `doneWhenProofs = [{id, title, cmd}]`. Intent = the reference's context sections (`## Overview`/`## Requirements`/…). Or a `goal.md` charter (proofs from its audit table, intent from its stated outcome). **If tasks.md `## Needs your attention` has unresolved blocking `❓ NEEDS YOU` items, pass them in `intentNotes` — an open blocking decision means the plan isn't acceptable yet; surface each in the verdict's NOT-covered bucket rather than blessing around it.**
...
- **Input:** a `/blueprint` plan (a `<spec-stem>/` directory with `tasks.md` + `reference.md`, or a single `<spec>.md`), a `goal-prep` `goal.md` charter, or — for **small tasks** — a native plan-mode / inline plan straight from the conversation (`native plan mode → implement → /verify-done`). `goal-prep` writes "hand finished work to `/verify-done`" into the charter; this is that hand-off.

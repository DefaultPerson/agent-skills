...
4. **Search terms** — for each term: `bash scripts/svgl.sh search "<term>" [N]`.
   ...
   - Else (>1, no exact) → `AskUserQuestion` listing up to 4 candidates (title — category); if more, note "refine the query or use `--all`". With `--all`, take all matches.
...
- Standalone utility — not part of the cleanup→clarify flow and **does not call** other skills.

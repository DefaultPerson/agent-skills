...
description: >
  ...
  concurrent `codex exec -` subprocesses ("Design It Twice"). Use for
  architectural friction: hard-to-test modules, tangled callers, refactors
  that feel obvious but the right shape isn't. Triggers: "deepen",
  "/deepen", "improve architecture", "refactor this module", "make X
  more testable".
when_to_use: >
...
This is the **Codex CLI variant**. Behaviourally identical to the Claude variant — user-facing prompts use numbered TUI input instead of `AskUserQuestion`; Phase 1 codebase exploration runs with native Bash/Glob/Grep/Read instead of `Agent(subagent_type="Explore")`; Phase 3 "Design It Twice" parallel exploration uses three concurrent `codex exec -` subprocesses instead of the `Agent` tool. Shared resources (`references/architecture-language.md`, `references/deepening-patterns.md`, `references/parallel-interface-design.md`) are symlinked from the Claude variant tree via `install-codex.sh`.
...
- **Non-interactive mode (`codex exec` invocation of `/deepen` itself).** Phase 2 candidate picking and Phase 4 ADR offers need TUI input. From `codex exec` without TTY, print the candidate list and stop rather than auto-proceeding past the user gate.
...
Then surface a numbered TUI prompt: "Which would you like to explore?" — options are the numbered candidates + Skip.
...
1. Spawn 3 sub-agents in parallel via three concurrent `codex exec -` subprocesses, each with a different design constraint:
   ...
   ```bash
   printf '%s' "$BRIEF_A" > /tmp/deepen-A.txt
   printf '%s' "$BRIEF_B" > /tmp/deepen-B.txt
   printf '%s' "$BRIEF_C" > /tmp/deepen-C.txt
   ( codex exec - < /tmp/deepen-A.txt > /tmp/deepen-A.out & )
   ( codex exec - < /tmp/deepen-B.txt > /tmp/deepen-B.out & )
   ( codex exec - < /tmp/deepen-C.txt > /tmp/deepen-C.out & )
   wait
   ```
...
- **If hard-to-reverse and surprising and a real trade-off** (mattpocock's ADR criteria) — offer to create `docs/adr/NNNN-slug.md` per `/blueprint`'s `references/adr-format.md` (shared file). Three-line ADR is enough.
- **Print a summary** to stdout: chosen candidate + the design (if Phase 3 ran). The user takes it to `mattpocock:tdd`, manual implementation, or `codex exec`-driven build.

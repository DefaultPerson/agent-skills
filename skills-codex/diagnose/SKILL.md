...
This is the **Codex CLI variant**. Behaviourally identical to the Claude variant — user-facing prompts use numbered TUI input instead of `AskUserQuestion`, and explorations run with Bash/Read/Grep directly instead of an `Agent` subagent. The narrative is otherwise the same.
...
- **Non-interactive mode (`codex exec` invocation of `/diagnose` itself).** Phase 3 user check-in (showing the ranked hypotheses for re-ranking) needs a TUI. From `codex exec` without TTY, proceed with your own ranking but state the assumption in the output rather than silently auto-proceeding.
...
✅ **Right:** Write 3-5 ranked hypotheses BEFORE testing any. Each one falsifiable: "If X is the cause, then changing Y will make the bug disappear / changing Z will make it worse." Show the list to the user via numbered TUI prompt — they often know which is most likely from context.
...
Stop and say so explicitly. List what you tried. Ask the user via numbered TUI prompt for: (a) access to an environment that reproduces it, (b) a captured artifact (HAR file, log dump, core dump, screen recording with timestamps), or (c) permission to add temporary production instrumentation. **Do not proceed to hypothesise without a loop.**
...
**Show the ranked list to the user before testing**, via numbered TUI prompt. They often have domain knowledge that re-ranks instantly ("we just deployed a change to #3"), or know hypotheses they've already ruled out. Cheap checkpoint, big time saver. Don't block on it — proceed with your ranking if the user is AFK.

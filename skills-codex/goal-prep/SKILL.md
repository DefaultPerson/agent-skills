...
name: goal-prep
description: >
  Codex CLI variant. Compile a charter for Codex's native /goal command.
  Runs an Intake Compiler (11 fields), a diagnostic ladder (6 questions
  in 3 batches), classifies the goal, seeds per-kind anti-patterns, and
...
# /goal-prep (Codex variant)

Companion to Codex's native `/goal` command. Adapted from
`tolibear/goalbuddy` `$goal-prep` (MIT). Same architectural strut: prep
is a charter compiler, never a lightweight executor. The Codex-side
variant of `iron-skills/skills/goal-prep/SKILL.md` (Claude Code
variant lives at the root of the same repo).

## Why the on-disk charter pattern

Native `/goal <condition>` in Codex is a Stop-hook wrapper with a
small-fast-model evaluator that runs after every turn. The evaluator
sees **only conversation context** — files on disk are invisible to it.
So the **outcome predicates** (what the evaluator checks) must live in
the `/goal` condition string itself.
...
needed (pre-v0.3.3 versions of this skill that did paste one were
...
The 4000-char ceiling is **upstream**, not ours: native Codex `/goal`
enforces it (`MAX_THREAD_GOAL_OBJECTIVE_CHARS` in openai/codex) and
Claude Code's `/goal` uses the same constant. We mirror it at intake so
...
This Codex CLI variant uses `claude -p` (Claude in headless mode) as
the cross-LLM hook. The mirror Claude Code variant under the root
`skills/goal-prep/SKILL.md` uses `codex review --uncommitted` instead —
...
AskUserQuestion({
  questions: [{
    question: "Include `claude -p \"<prompt>\"` as a pre-completion step in the goal charter? Runs an adversarial code review via the Claude CLI (a different model) against the working-tree diff before declaring the goal complete. Requires the claude CLI on PATH.",
    header: "Cross-LLM review",
    multiSelect: false,
    options: [
      // First option is Recommended based on CLAUDE_ON_PATH (1 → Yes first, 0 → No first).
      {label: <"Yes — include claude -p review hint (Recommended)" if detected else "No — skip cross-LLM review (Recommended)">,
       description: "Charter ## Adversarial Review will contain Step 1 (audit table) + Step 2 (claude -p pass)."},
      {label: <opposite>,
...
`claude -p "<prompt>"` returns a single response and exits. If
installed and authenticated, this does a second-opinion review with a
different model, catching what the main /goal model misses.
```
...
dotfile dir, separate from Claude Code's `.claude/` or Codex's
`~/.codex/` state areas. User can commit it
(`git add .ags/<slug>/goal.md`) if the goal is part of project
history; otherwise add `.ags/` to `.gitignore`.

**Backward compat (v0.3.2 path move).** If `<cwd>/.ags/<slug>/` does
not exist but `<cwd>/.claude/ags/<slug>/` does (legacy v0.3.1 path),
treat it as the existing slug for the prompt below; on "Resume" use it
in place, on "Archive/Fork" migrate to the new `.ags/` path.
...
2. **Adversarial review pass** — run via Bash:
   `git diff | claude -p "<prompt>"` (or `claude -p` reading the diff
   via stdin) for a second-opinion review from a different model.
   Review findings explicitly, no auto-apply. Headless mode (`-p`) keeps
   the review a one-shot call rather than spawning an interactive
   session.
...
AskUserQuestion, do **not** auto-stage to clipboard. After printing
the copy-clear-paste block above the skill exits. Once the user types
`/clear` a brand-new session starts — no follow-up question would
reach them anyway.

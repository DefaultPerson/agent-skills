...
## When /blueprint writes here

Only when the **Scope-cut audit** (Phase 5) resolves a deferral as **"Drop entirely (firm)"** — not a `[later]` tag — and the user explicitly opts into recording the rejection. Treat firm drops as durable; tentative deferrals stay in plan metadata.
...
## When /blueprint reads `.out-of-scope/`
...
- Not a TODO list for v2 work — use the plan's own `[later]` tagging for that.
